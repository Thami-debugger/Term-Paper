...
\paragraph{Results.}
Our reproduction is currently \textbf{partial rather than blocked}. After fixing solver/WSL integration and scoring, planner-backed runs execute successfully on validated sampled instances (for example, the campus domain at 10\% observations for all three planner profiles). However, full Table~1 replication remains incomplete because many domain/observation cells are still missing in the current run set.
...
We used the original benchmark archives provided at the paper's project website (\url{https://sites.google.com/site/prasplanning}), stored in \texttt{benchmarks/experiments/}. The site's ``Benchmarks \& Software uploaded'' page states that the benchmarks and software used in the experiments were uploaded in the site's Files section, which is the provenance for the benchmark bundle used here. Each domain has five observation levels (10\%, 30\%, 50\%, 70\%, 100\%). We ran \texttt{max\_instances=1} per obs-level per domain due to computational constraints (WSL overhead).
...
\subsection{Planner Execution Status}

Planner execution is now working through a configured WSL distro for sampled runs, but coverage is still incomplete across all domain/observation/planner combinations needed for full paper replication. Failed cells are surfaced explicitly and skipped, rather than being converted into fabricated uniform-posteriors outputs.

\subsection{Runtime Results}

The framework now returns real runtimes and scores for completed cells. Table~\ref{tab:our-results} shows currently available results from the sampled run set; missing entries correspond to cells not yet completed successfully.
